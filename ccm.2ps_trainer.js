...
(() => {
  const component = {
    ...
    config: {
      ...
      toposort: [
        "ccm.load",
        { url: "./libs/toposort/toposort.js#toposort", type: "module" },
      ],
      value: [1, 9],
    },
    Instance: function () {
      ...
      this.next = () => {
        ...
        if (!section.steps.length) {
          ...
          schedule.rules.forEach(([op1, op2]) => {
            for (let i = 1; i <= 2; i++)
              ["A", "B"].forEach((attr) =>
                rules.push([op1 + i + attr, op2 + i + attr])
              );
          });
          const history = data.sections.map((section) =>
            section.steps.toString()
          );
          const random = Math.random() * 100;
          for (const key in schedule.inputs)
            if (random >= schedule.inputs[key]) section.solution = key;
          let repeats = 0;
          do {
            section.steps = this.toposort($.shuffleArray(steps), rules).map(
              (step) => {
                if (step[0] === "o")
                  step +=
                    (randomFromRange(0, 1) ? "+" : "*") +
                    randomFromRange(...this.operand);
                return step;
              }
            );
          } while (
            (history.includes(section.steps.toString()) ||
              getSolution(section) !== section.solution) &&
            ++repeats <= 500
          );
        ...
        this.html.render(
          this.html.main(this, this.section2table(section)),
          this.element
        );
        // this.html.render(
      ...
      const getSolution = (section) => {
        ...
        function serializable() {
          ...
          t21 = solve(t21);
          return (
            ((t__.A === t12.A && t__.B === t12.B) ||
              (t__.A === t21.A && t__.B === t21.B)) &&
            "serializable"
          );
